...
import com.haibin.calendarview.LunarUtil.solarToLunar
import java.util.Calendar

object LunarCalendar {
	private val SPECIAL_FESTIVAL: MutableMap<Int?, Array<String?>?> = mutableMapOf()
	private val SOLAR_TERMS: MutableMap<Int?, Array<String?>?> = mutableMapOf()
	 ...
	 * 闰九月 农历正月初一对应公历1月31号
	 */
	private val LUNAR_INFO = intArrayOf(0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,  //1900-1909
	 ...
	 * 特殊节日的数组
	 */
	private lateinit var SPECIAL_FESTIVAL_STR: Array<String>
	 ...
	 * @return 数字转换为汉字月份
	 */
	private fun numToChineseMonth(month: Int, leap: Int): String = if (leap == 1) "闰" + MONTH_STR[month - 1]
	...
	private fun numToChinese(month: Int, day: Int, leap: Int): String =
		if (day == 1) numToChineseMonth(month, leap)
		else DAY_STR[day - 1]
	...
	fun daysInLunarMonth(year: Int, month: Int): Int =
		if ((LUNAR_INFO[year - CalendarViewDelegate.MIN_YEAR] and (0x10000 shr month)) == 0) 29
		else 30
	...
	private fun getSolarTerm(year: Int, month: Int, day: Int): String {
		...
		val text = "$year" + getString(month, day)
		var solar = ""
		checkNotNull(solarTerm)
	...
	private fun getSpecialFestival(year: Int, month: Int, day: Int): String {
		if (!SPECIAL_FESTIVAL.containsKey(year)) {
			SPECIAL_FESTIVAL[year] = getSpecialFestivals(year)
		}
		val specialFestivals = SPECIAL_FESTIVAL[year]
		val text = "$year" + getString(month, day)
		var solar = ""
		checkNotNull(specialFestivals)
	...
	private fun getSpecialFestivals(year: Int): Array<String?> {
		...
		date.set(year, 4, 1)
		var week = date.get(Calendar.DAY_OF_WEEK)
		var startDiff = 7 - week + 1
		festivals[0] = (if (startDiff == 7) dateToString(year, 5, startDiff + 1) + SPECIAL_FESTIVAL_STR[0]
		else dateToString(year, 5, startDiff + 7 + 1) + SPECIAL_FESTIVAL_STR[0])
		date.set(year, 5, 1)
		week = date.get(Calendar.DAY_OF_WEEK)
		startDiff = 7 - week + 1
		festivals[1] = (if (startDiff == 7) dateToString(year, 6, startDiff + 7 + 1) + SPECIAL_FESTIVAL_STR[1]
		else dateToString(year, 6, startDiff + 7 + 7 + 1) + SPECIAL_FESTIVAL_STR[1])
		date.set(year, 10, 1)
		week = date.get(Calendar.DAY_OF_WEEK)
		startDiff = 7 - week + 1
		festivals[2] = (if (startDiff <= 2) dateToString(year, 11, startDiff + 21 + 5) + SPECIAL_FESTIVAL_STR[2]
		else dateToString(year, 11, startDiff + 14 + 5) + SPECIAL_FESTIVAL_STR[2])
		return festivals
	}
	
	private fun dateToString(year: Int, month: Int, day: Int): String =
		"$year" + getString(month, day)
