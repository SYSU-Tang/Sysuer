...
object LunarUtil {
	 ...
	 */
	@JvmStatic fun solarToLunar(year: Int, month: Int, day: Int): IntArray {
		var index = year - SOLAR[0]
		if (SOLAR[index] > (year shl 9) or (month shl 5) or (day)) index--
		var offset = solarToInt(year, month, day) - solarToInt(getBitInt(SOLAR[index], 12, 9), getBitInt(SOLAR[index], 4, 5), getBitInt(SOLAR[index], 5, 0))
		...
		offset += 1
		(0..12).forEach {
			val dm = if (getBitInt(days, 1, 12 - it) == 1) 30 else 29
			if (offset > dm) {
				lunarM++
				offset -= dm.toLong()
			}
	...
	fun lunarToSolar(lunarYear: Int, lunarMonth: Int, lunarDay: Int, isLeap: Boolean): IntArray {
		...
		var loop = leap
		if (!isLeap) {
			loop = if (lunarMonth <= leap || leap == 0) lunarMonth - 1
			else lunarMonth
		}
		(0..<loop).forEach {
			offset += if (getBitInt(days, 1, 12 - it) == 1) 30 else 29
		}
		offset += lunarDay
		return solarFromInt(solarToInt(getBitInt(SOLAR[lunarYear - SOLAR[0]], 12, 9), getBitInt(SOLAR[lunarYear - SOLAR[0]], 4, 5), getBitInt(SOLAR[lunarYear - SOLAR[0]], 5, 0)) + offset - 1)
	}
}
