...
class CrashActivity : BaseActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		binding!!.copy.setOnClickListener { _: View? ->
			config.copy("crash", crash.getValue())
		...
		crash.observe(this, Observer { s: String? ->
			Markwon.builder(this).usePlugin(TablePlugin.create(this)).build()
				.setMarkdown(binding!!.crashContent, s!!)
		})
		...
		try {
			...
			if (crashInfo != null && !crashInfo!!.isEmpty()) {
				...
				if (lines.isNotEmpty()) {
					...
					if (firstLine.contains(":")) exceptionType = firstLine.split(":".toRegex())
			...
			config.copy("crash_issue", crash.getValue())
			config.toast(R.string.copy_successfully)
			
			startActivity(Intent(Intent.ACTION_VIEW).setData(githubUrl.toUri())
	...
	val availableMemory: String
		get() {
			...
			return String.format(Locale.getDefault(), "%.2f MB / %.2f MB", (memoryInfo.availMem / (1024.0 * 1024.0)), (memoryInfo.totalMem / (1024.0 * 1024.0)))
		}
	val storageInfo: String
		get() {
			try {
				val statFs = StatFs(Environment.getDataDirectory().path)
				return String.format(Locale.getDefault(), "%.2f GB / %.2f GB", (statFs.availableBlocksLong * statFs.blockSizeLong / (1024.0 * 1024.0 * 1024.0)), (statFs.blockCountLong * statFs.blockSizeLong / (1024.0 * 1024.0 * 1024.0)))
			} catch (_: Exception) {
				return "Unknown"
			}
		}
	...
	fun createDetailedIssueBody(throwable: Throwable): String {
		val markdown = StringBuilder()		// 用户描述
		...
		try {
			...
			markdown.append("| 应用版本 | ").append(packageInfo.versionName).append(" (")
				.append(PackageInfoCompat.getLongVersionCode(packageInfo)).append(") |\n")
			markdown.append("| 包名 | ").append(packageName).append(" |\n")
		} catch (e: PackageManager.NameNotFoundException) {
			throw RuntimeException(e)
		}
		markdown.append("| 构建类型 | ").append(BuildConfig.BUILD_TYPE)
			.append(" |\n\n")		// 设备信息表格
		markdown.append("## 📱 设备信息\n")
		markdown.append("| 项目 | 值 |\n")
		markdown.append("|------|-----|\n")
		markdown.append("| 设备型号 | ").append(Build.MANUFACTURER).append(" ").append(Build.MODEL)
			.append(" |\n")
		markdown.append("| Android版本 | ").append(Build.VERSION.RELEASE).append(" (API ")
			.append(SDK_INT).append(") |\n")		// 屏幕信息
		if (SDK_INT >= Build.VERSION_CODES.R) {
			val bounds = windowManager?.currentWindowMetrics?.bounds
			bounds?.let {
				markdown.append("| 屏幕分辨率 | ").append(it.width()).append("×")
					.append(it.height())
			}
		} else {
			val realSize = Point()
			windowManager?.defaultDisplay?.getRealSize(realSize)
			markdown.append("| 屏幕分辨率 | ").append(realSize.x).append("×").append(realSize.y)
				.append(" |\n")
		}
		markdown.append("| 屏幕密度 | ").append(resources.displayMetrics.densityDpi)
			.append("dpi |\n")
		markdown.append("| 时区 | ").append(TimeZone.getDefault().id).append(" |\n")
		markdown.append("| 语言 | ").append(Locale.getDefault().language)
			.append(" |\n\n")		// 崩溃详情
		markdown.append("## 💥 崩溃详情\n")
		markdown.append("**异常类型**: `").append(throwable.javaClass.getSimpleName())
			.append("`\n\n")
		markdown.append("**异常消息**: \n```txt\n")
			.append(if (throwable.message != null) throwable.message else "无消息")
			.append("\n```\n\n")		// 复现步骤
		...
		markdown.append(LocalDateTime.now()
			                .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
			.append("\n\n")
		return "$markdown"
	}
	...
	private val batteryStatus: String
		get() {
			...
			if (batteryStatus != null) {
				...
				return String.format(Locale.getDefault(), "%.1f%% %s", batteryPct, if (isCharging) "(充电中)" else "(未充电)")
			}
			return "未知"
		}
}