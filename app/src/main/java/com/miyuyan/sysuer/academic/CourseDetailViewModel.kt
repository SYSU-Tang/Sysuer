...
import com.miyuyan.sysuer.view.RowOrientation
	...
	fun downloadOutline(outlineId: String) {
		DownloadManager.downloadFile(
				...
				model.http.generateRequest(
						"https://jwxt.sysu.edu.cn/jwxt/training-programe/courseoutline/outlineupdateworddownload?outlineUpdateId=$outlineId",
						null,
						null
	...
	private fun updateOutlineSections(scheduleList: JSONArray) {
		...
		scheduleList.forEachIndexed { i, e ->
			if (e is JSONObject) {
				...
				_outlineSections.add(
						SectionData(
								title = "第${i + 1}章（${hours}${app.getString(R.string.study_hour)}）",
								rows = rows
						)
				)
			}
