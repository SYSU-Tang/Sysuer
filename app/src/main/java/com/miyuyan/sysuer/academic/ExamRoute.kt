...
fun ExamRoute(
	...
	val examWeekId by examViewModel.examWeekId.collectAsStateWithLifecycle()
	...
	val uiState by examViewModel.uiState.collectAsStateWithLifecycle()
	...
	LaunchedEffect(examWeekId, term) {
		if (examWeekId != null && term != null) examViewModel.getResult()
	}
	ActivityPager(
		...
		topBarContent = {
			Column(
				...
				ExposedDropdownMenuBox(
					expanded = termExpanded, onExpandedChange = { termExpanded = it }) {
					OutlinedTextField(
						modifier = Modifier
					...
					ExposedDropdownMenu(
						expanded = termExpanded, onDismissRequest = { termExpanded = false }) {
				...
				ExposedDropdownMenuBox(
					expanded = examExpanded, onExpandedChange = { examExpanded = it }) {
					OutlinedTextField(
						modifier = Modifier
							.menuAnchor(ExposedDropdownMenuAnchorType.PrimaryNotEditable)
							.fillMaxWidth(),
					...
					ExposedDropdownMenu(
						expanded = examExpanded, onDismissRequest = { examExpanded = false }) {
