...
class ExamViewModel(application: Application) : AndroidViewModel(application) {
	...
	private val _uiState: MutableStateFlow<UiState> = model.getUiState(4)
				...
				if (response.getInteger("code") == 200) {
					when (code) {
						...
						3 -> {
							...
							val finalExamWeek = response.getJSONArray("data")
								.firstOrNull { (it as JSONObject).getString("examWeekName") == "18-19周期末考" } as? JSONObject
						...
						4 -> {
							_sections.clear()
							if (response.getJSONArray("data").isEmpty()) _uiState.value =
								UiState.Empty else response.getJSONArray("data")
								?.forEach { a: Any? ->
									val timeTable = (a as JSONObject).getJSONObject("timetable")
									if (timeTable.isEmpty()) _uiState.value = UiState.Empty
									else _uiState.value = UiState.Content
									timeTable.keys.sorted().forEach {
										timeTable[it]?.let { item ->
											(item as JSONArray).forEach { o: Any? ->
												_sections.add(
													SectionData(
														(o as JSONObject).getString("examSubjectName"),
														rows = extractValue(
															application, o, intArrayOf(
																R.string.subject,
																R.string.exam_classroom,
																R.string.duration,
																R.string.date,
																R.string.year
															), arrayOf(
