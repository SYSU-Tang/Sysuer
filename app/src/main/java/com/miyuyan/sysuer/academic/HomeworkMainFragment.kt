...
import com.miyuyan.sysuer.databinding.ItemHomeworkBinding
...
class HomeworkMainFragment : BaseFragment() {
										...
										adapter.addAdapter(HomeworkAdapter().apply {
											...
											listener = object : AdapterListener {
												override fun onBind(
													...
													ItemHomeworkBinding.bind(holder.itemView)
														.apply {
															fold.setOnClickListener {
																detailDialog.setMessage(
																		Html.fromHtml(
																				get(
																						position
	...
	fun getLmsTask(key: String) {
		http.postRequest(
				"https://lms.sysu.edu.cn/lib/ajax/service.php?sesskey=$key&info=core_calendar_get_calendar_upcoming_view",
	...
	internal class HomeworkAdapter : RecyclerAdapter<JSONObject>() {
		...
		override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
			...
			ItemHomeworkBinding.bind(holder.itemView).apply {
				...
				detail.text =
					"${context.getString(R.string.type)} ${item.getString("normalisedeventtypetext")}\n${
						context.getString(R.string.link)
					} ${item.getString("viewurl")}"
			}
