...
import com.miyuyan.sysuer.databinding.DialogRegionBinding
...
class LeaveReturnRegistrationFragment : StaggerFragment() {
	...
	override fun onCreateView(
		...
		if (root == null) {
			...
			val leaveKeys: MutableList<String?> = mutableListOf(
					"假期去向", "预计离校时间", "预计返校时间", "去向类型", "交通工具", "外出地"
			)
			...
			dialogRegionBinding.country.recyclerView.setLayoutManager(
					LinearLayoutManager(
							requireContext()
					)
			)
			...
			dialogRegionBinding.country.recyclerView.overScrollMode =
				RecyclerView.OVER_SCROLL_ALWAYS
			dialogRegionBinding.province.recyclerView.layoutManager =
			...
			dialogRegionBinding.province.recyclerView.overScrollMode =
				RecyclerView.OVER_SCROLL_ALWAYS
			dialogRegionBinding.county.recyclerView.layoutManager =
				LinearLayoutManager(requireContext())
							...
							when (code) {
								0 -> {
									...
									if (data.containsKey("yjlxsj") && !data.getString("yjlxsj", "")
											.isEmpty()
									) leaveDate.value = LocalDate.parse(data.getString("yjlxsj"))
										.atStartOfDay(ZoneId.systemDefault()).toInstant()
										.toEpochMilli()
									if (data.containsKey("yjfxsj") && !data.getString("yjfxsj", "")
											.isEmpty()
									) returnDate.value = LocalDate.parse(data.getString("yjfxsj"))
										.atStartOfDay(ZoneId.systemDefault()).toInstant()
										.toEpochMilli()
									...
									leave.addAll(
											mutableListOf<String?>(
													"离校",
													data.getString("yjlxsj", ""),
													data.getString("yjfxsj", ""),
													data.getString("qxlx", ""),
													data.getString("jtgj", ""),
									...
									stay.addAll(
											mutableListOf<String?>(
													"留校", data.getString("lxyy", "")
											)
									)
									if ("0" == isStay) addSection(
											getString(R.string.registration), leaveKeys, leave
									)
									else addSection(
											getString(R.string.registration), stayKeys, stay
									)
										...
										Button(
												onClick = {
													if ("0" == isStay) save(
															...
															isStay,
															if (isEmpty<Long?>(leaveDate.value)) "" else Instant
																.ofEpochMilli(
																		leaveDate.value!!
															...
															if (isEmpty<Long?>(returnDate.value)) "" else Instant
																.ofEpochMilli(
																		returnDate.value!!
																).atZone(ZoneId.systemDefault())
																.format(
									...
									(0 until adapter.itemCount).forEach { rowPos ->
										adapter.setRowClickListener(rowPos) {
											if (rowPos == 0) {
												...
												val menu =
													PopupMenu(requireContext(), requireView())
												mutableListOf<String?>(
														"离校", "留校"
												).forEach { i: String? ->
													menu.menu.add(i).setOnMenuItemClickListener {
											...
											if (adapter.itemCount == 6) {
												when (rowPos) {
													3, 4 -> {
														...
														(if (rowPos == 4) transportation else destination)!!.forEach { e: Any? ->
															menu.menu
																.add((e as JSONObject).getString("label"))
												...
												if (rowPos == 1) {
													val menu =
														PopupMenu(requireContext(), requireView())
													resources
														.getStringArray(R.array.registration_info_keys)
														.forEach { i: String? ->
															menu.menu.add(i)
																.setOnMenuItemClickListener {
																	stay[rowPos] = i
								...
								3 -> {
									...
									response.getJSONArray("data").forEach { e: Any? ->
										countryAdapter.add(
												(e as JSONObject).getString("label")
										)
									}
									countryAdapter.action = { pos: Int? ->
										country = response.getJSONArray("data").getJSONObject(pos!!)
											.getString("value")
								...
								4 -> {
									...
									provinceAdapter.action = { pos: Int? ->
										province =
											response.getJSONArray("data").getJSONObject(pos!!)
												.getString("value")
										getCity(province)
									}
									getCity(province)
								...
								5 -> {
									...
									cityAdapter.action = { pos: Int? ->
										city = response.getJSONArray("data").getJSONObject(pos!!)
											.getString("value")
									}
	...
	fun save(
		...
		model.addAndNext(
				"jjrlfx/api/sm-jjrlfx/student/register",
				"{\"cjlfxgzId\":\"$id\",\"sflx\":\"$isStay\",\"yjlxsj\":\"$leaveTime\",\"yjfxsj\":\"$returnTime\",\"qxlx\":\"$leaveType\",\"jtgj\":\"$transportation\",\"wcd\":{\"gj\":\"$country\",\"sf\":\"$province\",\"cs\":\"$city\"},\"wcdgj\":\"$country\",\"wcdsf\":\"$province\",\"wcdcs\":\"$city\"}",
				6
		)
	}
	...
	fun save(id: String, isStay: String?, reason: String?) {
		model.addAndNext(
				"jjrlfx/api/sm-jjrlfx/student/register",
				"{\"cjlfxgzId\":\"$id\",\"sflx\":\"$isStay\",\"lxyy\":\"$reason\"}",
				6
		)
	}
