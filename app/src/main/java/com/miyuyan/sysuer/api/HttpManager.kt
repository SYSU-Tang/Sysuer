...
//				requestStatus[what] = Status.SUCCESS
	...
	fun generateRequest(url: String, data: String? = null, type: String? = null): Request.Builder {
		...
		else if (cookieManager != null) request.header(
				"Cookie", cookieManager?.toSimpleString(host) ?: ""
		)
		if (isAuthorizationRequired && authorizationJar != null) request.header(
				"Authorization", authorizationJar?.getAuthorization(host) ?: ""
		)
		if (authorization != null) request.header("Authorization", authorization ?: "")
		if (referer != null) request.header("Referer", referer ?: "")
		if (ua != null) request.header("User-Agent", ua ?: "")
		if (data != null) request.post(
				data.toRequestBody(
						(type ?: "application/json").toMediaType()
				)
		)
		if (isTokenRequired && authorizationJar != null) request.header(
				"token", authorizationJar?.getToken(host) ?: ""
		)
		header?.forEach { (name: String, value: String?) ->
			request.header(
					name, value ?: ""
			)
		}
	...
	fun postRequest(url: String, data: String?, what: Int) {
		sendRequest(generateRequest(url, data).build(), what)
	}
...
//		generateRequest(url, data, "application/json")
...
//		sendRequest(generateRequest(url).build(), what)
//	}
//
//	/**
//	 * 发送 DELETE 请求
...
//		sendRequest(generateRequest(url).delete().build(), what)
//	}
