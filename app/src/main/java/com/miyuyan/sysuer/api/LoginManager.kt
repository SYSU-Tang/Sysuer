...
import com.miyuyan.sysuer.rainClass.QrCode
...
class LoginManager(private val context: Context) {
	...
	val client: OkHttpClient = OkHttpClient.Builder().connectTimeout(TIMEOUT, TimeUnit.SECONDS)
		.readTimeout(TIMEOUT, TimeUnit.SECONDS).writeTimeout(TIMEOUT, TimeUnit.SECONDS)
		.followRedirects(true).cookieJar(cookieJar).build()
	val directClient: OkHttpClient =
		OkHttpClient.Builder().followRedirects(false).cookieJar(cookieJar).build()
	...
	private fun loginForGym(path: String): String {
		val url = if (path.startsWith("http")) path else casAuthorizationManager.host + path
		val response = client.newCall(
				Request.Builder().header("Accept", "application/json, text/plain, */*").header(
							"User-Agent",
	...
	private fun redirect(response: String?): String? {
		...
		return when {
			"0" == json.getString("code") -> if (json.containsKey("data")) json
				.getJSONObject("data").getString("redirect")
			else json.getString("redirect")
	...
	fun loginForKTP(username: String, password: String): Boolean {
		...
		if (type?.contains("application/json") == true) {
			...
			return if ("1000" == code || status == 1) {
				authorizationJar.setToken(
						"www.ketangpai.com", result.getJSONObject("data").getString("token")
				)
				true
			} else {
	...
	fun loginForYuketang(imageView: ImageView?) {
		CompletableFuture.supplyAsync {
			val loginMessage = QrCode(imageView).run() ?: return@supplyAsync false
			try {
				val body = JSONObject.of(
						"UserID", loginMessage.getString("UserID"),
						"Auth", loginMessage.getString("Auth")
				).toJSONString()
				val text = client.newCall(
						Request.Builder()
							.post(body.toRequestBody("application/json".toMediaTypeOrNull()))
							.url("https://www.yuketang.cn/pc/web_login").build()
				).execute().body.string()
				println(text)
				val response = JSONObject.parse(text)
				println(response)
	...
	private fun loginSysu(
		username: String?, password: String, service: String, captcha: String?
	): Boolean {
		try {
			...
			cookieJar.add(
					...
					Cookie.Builder().name("device_trust_Cookie").value("true")
						.domain("cas.sysu.edu.cn").build()
			)
			if (service.contains("webvpn")) {
				casAuthorizationManager.isAccessible = false
				val publicKey =
					JSONObject.parse(this.publicKey).getJSONObject("data").getJSONObject("param")
				...
				when (service) {
					...
					TargetUrl.PAY -> {
						...
						cookieJar.saveFromResponse(
								service.toHttpUrl(), listOf(
								Cookie.Builder().name("ibps-1.0.1-token").value(token)
									.domain("pay.sysu.edu.cn").build()
						)
						)
					}

					TargetUrl.ZHNY -> authorizationJar.setAuthorization(
							host, getZHNYAuthoritarian(service)
					)
	...
	@Throws(IOException::class)
	private fun loginForPortal() {
		val location = directClient.newCall(
	...
	@Throws(IOException::class)
	private fun getXGXTToken(service: String?, targetBaseUrl: String?) {
		client.newCall(
				Request.Builder().url(
							"${targetBaseUrl}sso/login?realm=sysuRealm&ticket=${
								getTicket(service)
							}&service=$service"
					)
	...
	@Throws(IOException::class)
	private fun getPayToken(service: String?): String {
		return JSONObject.parse(
				client.newCall(
						Request.Builder()
							.url("https://pay.sysu.edu.cn/client/api/client/auth/netId/login")
							.header("Referer", "https://pay.sysu.edu.cn/").post(
									("{\"key\":\"https://cas.sysu.edu.cn/cas/serviceValidate?service=https://pay.sysu.edu.cn/sso&ticket=${
										getTicket(service)
	...
	private val lmsToken: String?
		get() {
			val response = client.newCall(
					Request.Builder().url("https://lms.sysu.edu.cn/my/").build()
			).execute().body.string()
	...
	@Throws(IOException::class)
	private fun getZHNYAuthoritarian(service: String?): String = JSONObject.parse(
			client.newCall(
					Request.Builder().url(
								"https://zhny.sysu.edu.cn/kbp/auth/third/h5/casLogin/${
									getTicket(service)
								}"
						).build()
			).execute().body.string()
	...
	@Throws(Exception::class)
	private fun encrypt(
		publicKeyBase64: String?, plainText: String
	): String? {
		val keyFactory = KeyFactory.getInstance("RSA")
		val cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding")
		cipher.init(
				Cipher.ENCRYPT_MODE, keyFactory.generatePublic(
				X509EncodedKeySpec(
	...
	@Throws(IOException::class)
	fun getTicket(service: String?): String = directClient.newCall(
			Request.Builder().url("${casAuthorizationManager.host}/esc-sso/login?service=$service")
				.build()
	).execute().headers["Location"]?.toHttpUrl()?.queryParameter("ticket") ?: ""

	@Throws(IOException::class)
	fun getGymToken(targetBaseUrl: String) {
		...
		if (!filterChallenge.isEmpty() && !TextUtils.isEmpty(prefix)) cookieJar.saveFromResponse(
				...
				Cookie.Builder().domain(targetBaseUrl.toHttpUrl().host)
					...
					.value(Answer.encode(prefix, filterChallenge[0]!!.value)).build()
		)
		)
	}
	...
	fun getNewsAuthorization(url: String?): String? {
		return getAuthorization(
				Request.Builder().url("${casAuthorizationManager.host}/esc-sso/login?service=$url")
					.build()
		)
	}
	...
	internal object Answer {
		...
		fun hexSha1(input: String): String =
		...
		fun hexToBinary(hexStr: String): String {
			...
			hexStr.forEach { element ->
				binaryStr.append(
						String.format(
								"%4s", Integer.toBinaryString(
								element.digitToIntOrNull(16) ?: -1
						)
						).replace(' ', '0')
				)
			}
		 ...
		 */
		@Throws(NoSuchAlgorithmException::class)
		fun binSha1(input: String): String = hexToBinary(hexSha1(input))
		...
		@Throws(NoSuchAlgorithmException::class)
		fun findSuffix(
			prefix: String?, leadingZeroBit: Int
		): String {
			var cnt = 0
			while (true) {
				val suffix = Integer.toHexString(cnt)
				val hashBinary: String = binSha1(prefix + suffix)
				if (hashBinary.substring(
							0, leadingZeroBit
					) == "0".repeat(leadingZeroBit)
				) return suffix
				cnt++
			}
		}
		...
		@Throws(NoSuchAlgorithmException::class)
		fun getFinalCookie(
			safelineBotChallenge: String?, prefix: String?, leadingZeroBit: Int
		): String = safelineBotChallenge + findSuffix(prefix, leadingZeroBit)
	...
	internal object AESCBCEncrypter {
		...
		@Throws(Exception::class)
		fun encryptByCBC(
			plaintext: String, key: String, iv: String
		): String? {
			...
			cipher.init(
					Cipher.ENCRYPT_MODE, SecretKeySpec(keyBytes, "AES"), IvParameterSpec(ivBytes)
			)
			return Base64.getEncoder()
				.encodeToString(cipher.doFinal(plaintext.toByteArray(StandardCharsets.UTF_8)))
		}
