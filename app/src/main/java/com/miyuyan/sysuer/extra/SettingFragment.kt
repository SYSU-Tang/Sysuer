...
import com.miyuyan.sysuer.browser.BrowserPreference
import com.miyuyan.sysuer.preference.MenuPreference
...
class SettingFragment : PreferenceFragmentCompat() {
	override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
		...
		((findPreference("dynamic_color")) as MaterialSwitchPreference?)?.setOnPreferenceChangeListener { _: Preference?, _: Any? ->
			requireActivity().recreate()
			true
		}
		((findPreference("icon_theme")) as MenuPreference?)?.setOnPreferenceChangeListener { _: Preference?, newValue: Any? ->
			...
			with(requireActivity().packageManager) {
				...
				setComponentEnabledSetting(
					...
					(intArrayOf(1, 2, 2))[(newValue as String?)!!.toInt()],
				...
				setComponentEnabledSetting(
					...
					), (intArrayOf(2, 1, 2))[newValue.toInt()], PackageManager.DONT_KILL_APP
				)
				setComponentEnabledSetting(
					ComponentName(
						requireActivity().baseContext,
						"$pkg.MainActivityDefault"
					), (intArrayOf(2, 2, 1))[newValue.toInt()], PackageManager.DONT_KILL_APP
				)
			}
			true
		}
		...
		((findPreference("image_blocked")) as MaterialSwitchPreference?)?.setChecked(
			browserPreference.isImageBlocked
		)
		...
		((findPreference("save_mobile_data_mode")) as MaterialSwitchPreference?)?.setChecked(
			browserPreference.isSaveMobileDataMode
		)
		...
		((findPreference("privacy_mode")) as MaterialSwitchPreference?)?.setChecked(
			browserPreference.isPrivacyMode
		)
		...
		(findPreference("developer_mode") as Preference?)?.isVisible =
