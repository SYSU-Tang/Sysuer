...
class ComplaintModel(val model: XinfangModel) {
	 ...
	 * @return 是否通过校验
	 */
	fun isValidateNotEmpty(
		fields: MutableMap<String?, String?>, requiredKeys: MutableSet<String?> = mutableSetOf(
				"visitorName", "phone", "mobileCheckCode", "name", "description", "checkCode"
		), fieldLabels: MutableMap<String?, String?> = mutableMapOf(
				"visitorName" to "姓名",
				"phone" to "手机号",
				"mobileCheckCode" to "短信验证码",
				"name" to "反映主题",
				"description" to "反映内容",
				"checkCode" to "验证码"
		)
	...
	fun isValidateForm(phone: String?, description: String?): Boolean = when {
		isInvalidPhone(phone) -> {
			model.toast("手机号码填写有误,请输入有效的11位手机号码!")
			false
		}

		description != null && description.length > 1000 -> {
			model.toast("反映内容不能多于1000字!")
			false
		}
	...
	fun canSendSms(): Boolean =
		if (lastSmsSendTime > 0 && (System.currentTimeMillis() - lastSmsSendTime) < 60000) {
			model.toast("两次获取验证码必须间隔一分钟以上!")
			false
		} else true
	...
	fun sendMobileCode(phone: String?) {
		if (isInvalidPhone(phone)) {
			...
			model.call(
					"servlet/executeFun?className=MobileCode&ajaxType=get&function=sendMobileCode&type=mobile&subtype=jsjb&mobileNum=${
						URLEncoder.encode(
								phone, "utf8"
						)
					}", "", null, object : Callback {
				override fun onFailure(call: Call, e: okio.IOException) {
					model.toast("短信验证码发送失败! HTTP " + e.message)
				}

				override fun onResponse(call: Call, response: Response) {
					if (!response.isSuccessful) {
						model.toast("短信验证码发送失败! HTTP " + response.code)
						return
					}
					val responseText = response.body.string()
					if (responseText.startsWith("syserror_")) {
						val errorMsg = responseText.substring("syserror_".length)
						model.toast("短信验证码发送失败:$errorMsg")
					} else {
						lastSmsSendTime = System.currentTimeMillis() // 更新发送时间
						println("发送成功, 响应: $responseText")
						model.toast("已发送短信验证码!")
					}
				}
			})
		}
	...
	fun submitForm(
		...
		if (isValidateNotEmpty(formFields)) {
			val jsonBody =
				JSONObject.of("c", JSONObject(formFields), "h", hiddenFields, "f", attachments)
					.toJSONString()
			//println("Request Body: $jsonBody")
			model.enqueue("jsp_api/fywt", jsonBody, 2)
		}
	}
