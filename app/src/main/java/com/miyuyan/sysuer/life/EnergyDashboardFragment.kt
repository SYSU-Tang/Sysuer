...
//			println("code = $code , response = $response")
					if (response.getInteger("code") == 200) {
						...
						when (code) {
							...
							1 -> {
								binding.lastMonthElectricity.text = String.format(
										Locale.getDefault(),
										"上月用电\n%.2f",
										data.getDouble("lastMonthUsage")
								)
								binding.thisMonthElectricity.text = String.format(
										Locale.getDefault(),
										"本月用电\n%.2f",
										data.getDouble("currentMonthUsage")
								)
								binding.deltaElectricity.text = String.format(
										Locale.getDefault(),
										"用电变化\n%.2f",
										data.getDouble("usageChange")
								)
							}

							2 -> {
								binding.lastMonthWater.text = String.format(
										Locale.getDefault(),
										"上月用水\n%.2f",
										data.getDouble("lastMonthUsage")
								)
								binding.thisMonthWater.text = String.format(
										Locale.getDefault(),
										"本月用水\n%.2f",
										data.getDouble("thisMonthUsage")
								)
								binding.deltaWater.text = String.format(
										Locale.getDefault(),
										"用水变化\n%.2f",
										data.getDouble("growthUsage")
								)
							}

							3 -> {
								reset()
								response.getJSONObject("data").getJSONArray("records")
									.forEach { item: Any? ->
										val titleAdapter =
											TitleAdapter((item as JSONObject).getString("date"))
										titleAdapter.header = 2
										adapter.addAdapter(titleAdapter)
										item.getJSONArray("detailRecords").forEach { detail: Any? ->
											adapter.addAdapter(
													TitleAdapter(
															(detail as JSONObject).getString(
																	"tradeTypeDesc"
															)
													)
											)
							...
							4 -> {
								...
								if (!rooms.isEmpty()) getOrderInfo(
										rooms.valueAt(0)!!.second,
										LocalDate.now()
