				...
				model.message.collect { (code, response) ->
					when (code) {
						0 -> {
							...
							cashAdapter.set(
									mutableListOf(R.string.sport_credit, R.string.wallet),
							...
							arrayOf("validSwimmer", "IsAdmin").forEachIndexed { i, v ->
								idAdapter.add(
										getString(
												listOf(
														R.string.is_swimmer_valid, R.string.admin
												)[i]
										), if (response.getBoolean(v)) getString(R.string.yes)
								else getString(R.string.no), R.drawable.help
								)
							}
						...
						1 -> response.getJSONArray("data").forEach { i: Any? ->
							...
							val list: ArrayList<String?> = extractValue(
									item, arrayOf(
									"Status", "ValidUntil", "PhysicalExamDate"
							)
							)
							...
							certAdapter.set(
									...
									if ("approved" == list[0]) R.drawable.uncheck else R.drawable.check,
									R.drawable.calendar,
									R.drawable.calendar
							), requireContext()
							)
