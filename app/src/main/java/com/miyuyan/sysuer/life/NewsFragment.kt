...
import com.miyuyan.sysuer.databinding.ItemNewsBinding
		...
		val newsAdapter = NewsAdapter().apply {
			...
			listener = object : AdapterListener {
				override fun onBind(
					...
					if (!img.isEmpty()) Glide.with(context).load(
							GlideUrl(
									img,
									LazyHeaders.Builder().addHeader("Cookie", model.cookie)
										.addHeader("Authorization", model.authorization).build()
							)
					).timeout(15000).override(config.dpToPx(120), config.dpToPx(120))
						.optionalFitCenter().transform(RoundedCorners(16)).into(binding.image)
				}
		...
		binding = RecyclerViewScrollBinding.inflate(inflater).apply {
			...
			recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
				override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
					if (!recyclerView.canScrollVertically(1) && position != 0 && dy > 0) getData()
				}
			})
			...
			repeatOnLifecycle(Lifecycle.State.STARTED) {
				model.messageChannel.receiveAsFlow().collect { (code, response) ->
					(if (code == 3) response.getJSONArray("data")
						...
						val image: String =
							(item as JSONObject).getJSONArray("coversPicList")?.let {
								if (it.isNotEmpty() && !it.getJSONObject(0).isNullOrEmpty()) it
									.getJSONObject(0).getString("outLink")
								else ""
							} ?: ""
						newsAdapter.add(
								mutableMapOf(
