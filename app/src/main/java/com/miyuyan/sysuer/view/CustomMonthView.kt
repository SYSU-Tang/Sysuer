...
class CustomMonthView(context: Context) : MonthView(context) {
	...
	override fun onDrawSelected(canvas: Canvas,
		...
		canvas.drawCircle((x + mItemWidth / 2).toFloat(), (y + mItemHeight / 2).toFloat(), mRadius.toFloat(), mSelectedPaint)
		return true
	}
	
	override fun onDrawScheme(canvas: Canvas, calendar: Calendar, x: Int, y: Int) {
		mPointPaint.setColor(if (isSelected(calendar)) Color.WHITE else Color.GRAY)
		canvas.drawCircle(x + mItemWidth.toFloat() / 2, (y + mItemHeight - 3 * mPadding).toFloat(), mPointRadius, mPointPaint)
	}
	...
	override fun onDrawText(canvas: Canvas,
		...
		if (isSelected) {
			...
			canvas.drawText(calendar.lunar, cx.toFloat(), mTextBaseLine + y + mItemHeight.toFloat() / 10, mSelectedLunarTextPaint)
		}
		else if (hasScheme) {
			...
			canvas.drawText(calendar.lunar, cx.toFloat(), mTextBaseLine + y + mItemHeight.toFloat() / 10, if (!TextUtils.isEmpty(calendar.solarTerm)) mSolarTermTextPaint else mSchemeLunarTextPaint)
		}
		else {
