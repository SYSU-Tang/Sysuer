...
class TomorrowClassWidget : AppWidgetProvider() {
			...
			handler = object : Handler(Looper.getMainLooper()) {
				override fun handleMessage(msg: Message) {
					if (msg.what != -1 && msg.getData().getBoolean("isJSON")) {
						...
						val remoteViews =
							RemoteViews(context.packageName, R.layout.widget_today_class)
						if (response.getJSONObject("meta").getInteger("statusCode") == 200) {
							...
							if (msg.what == 0) {
								...
								if (data.containsKey("list") && data.get("list") != null) {
									...
									val workRequest =
										OneTimeWorkRequest.Builder(DailyWidgetWorker::class.java)
											.setConstraints(
													Constraints.Builder()
														.setRequiredNetworkType(NetworkType.CONNECTED)
														.build()
											).setInitialDelay(
													(24 - LocalTime.now().hour).toLong(),
													TimeUnit.HOURS
											).build()
							...
							remoteViews.setTextViewText(
									R.id.widget_name, context.getString(R.string.tomorrow_class)
							)
	...
	private fun tomorrowSchedule() {
		...
		http.postRequest(
				"https://mportal.sysu.edu.cn/newClient/api/schedule/newSchedule/getNextDaySchedule",
				"{\"types\":[],\"startTime\":\"$tomorrow\",\"endTime\":\"${tomorrow.plusDays(1)}\"}",
				0
		)
	}
}
