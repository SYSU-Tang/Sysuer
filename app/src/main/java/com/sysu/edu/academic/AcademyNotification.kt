		...
		setContent {
			...
			LaunchedEffect(noticeContent) {
				noticeContent?.let { content ->
					val intent = Intent(this@AcademyNotification, BrowserActivity::class.java).putExtra("data", ("""<!DOCTYPE html><html><head>
																			  <style>
                                            body{
					...
					val options = sharedElementBounds?.let { bounds ->
						val view = android.view.View(this@AcademyNotification).apply {
							x = bounds.left
							y = bounds.top
							layoutParams = android.view.ViewGroup.LayoutParams(bounds.width.toInt(), bounds.height.toInt())
							transitionName = "miniapp"
						}
						(window.decorView as android.view.ViewGroup).addView(view)
						val opt = ActivityOptionsCompat.makeSceneTransitionAnimation(this@AcademyNotification, view, "miniapp").toBundle()
						window.decorView.postDelayed({ (window.decorView as android.view.ViewGroup).removeView(view) }, 1000)
						opt
					} ?: ActivityOptionsCompat.makeSceneTransitionAnimation(this@AcademyNotification, android.view.View(this@AcademyNotification), "miniapp").toBundle()
			...
			SharedTransitionLayout {
				ActivityPager(title = stringResource(id = R.string.academic_affair_notice),
				              tabs = mutableListOf(MenuItem(stringResource(id = R.string.academic_affair_notice)), MenuItem(stringResource(id = R.string.school_affair_notice))),
				              onNavigationClick = { supportFinishAfterTransition() },
				              pageContent = { page ->
					              AnimatedContent(targetState = page, label = "page_transition") { targetPage ->
						              NewsList(newsList = if (targetPage == 0) academicNotices else schoolNotices, sharedTransitionScope = this@SharedTransitionLayout, animatedVisibilityScope = this@AnimatedContent) { notice, bounds ->
...
@OptIn(ExperimentalSharedTransitionApi::class) @Composable fun NewsList(
	...
	LazyVerticalStaggeredGrid(columns = StaggeredGridCells.Adaptive(240.dp),
		...
		items(newsList, key = { it.getString("id") }) { item ->
			NewsItem(item = item, sharedTransitionScope = sharedTransitionScope, animatedVisibilityScope = animatedVisibilityScope) { bounds ->
...
@OptIn(ExperimentalSharedTransitionApi::class) @Composable fun NewsItem(
	item: JSONObject,
	sharedTransitionScope: SharedTransitionScope,
	animatedVisibilityScope: AnimatedVisibilityScope,
	onClick: (Rect) -> Unit,
	...
	with(sharedTransitionScope) {
		Card(onClick = { onClick(currentBounds) },
		     modifier = Modifier
			     .fillMaxWidth()
			...
			Column(modifier = Modifier
				...
				.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(dimensionResource(R.dimen.vertical_padding))) {
				Text(text = item.getString("title", ""), style = MaterialTheme.typography.titleMedium)
				Text(
