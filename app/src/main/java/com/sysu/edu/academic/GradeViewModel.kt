...
class GradeViewModel(application: Application) : AndroidViewModel(application) {
            ...
            if (response.getInteger("code") == 200) {
                when (code) {
                    ...
                    4 -> {
                        ...
                        fun createRows(names: IntArray, values: List<String?>): SnapshotStateList<RowData> {
                            val rows = mutableStateListOf<RowData>()
                            names.forEachIndexed { index, nameRes ->
                                rows.add(RowData(application.getString(nameRes), values.getOrNull(index) ?: ""))
                            }
                            return rows
                        }
                        ...
                        sections.add(SectionData(
                            ...
                            rows = createRows(
                                intArrayOf(R.string.total_rank, R.string.total_credit, R.string.total_point),
                                listOf("$totalRank/$total=${totalRank.toFloat()/total.toFloat()}", totalCredit, totalPoint)
                            )
                        ))
                        sections.add(SectionData(
                            ...
                            rows = createRows(
                                ...
                                listOf("$rank/$total=${rank.toFloat()/total.toFloat()}", point)
                            )
                        ))
                        sections.add(SectionData(
                ...
                model.contextUtil.toast(String.format(application.getString(R.string.arrears_warning), response.getString("message")))
            }
        }
    }
    ...
    fun fetchScore() {
        if (year != null && trainType != null) {
            ...
            model.addAndNext("jwxt/achievement-manage/score-check/list?scoSchoolYear=$year&trainTypeCode=$trainType&addScoreFlag=true&scoSemester=$termValue", 1)
            model.addAndNext("jwxt/achievement-manage/score-check/getSortByYear?scoSchoolYear=${year ?: ""}&trainTypeCode=$trainType&addScoreFlag=true&scoSemester=$termValue", 4)
        }
    }

    fun fetchAllYear() {
        if (trainType != null) {
            model.addAndNext("jwxt/achievement-manage/score-check/list?trainTypeCode=$trainType&addScoreFlag=true", 1)
            model.addAndNext("jwxt/achievement-manage/score-check/getSortByYear?trainTypeCode=$trainType&addScoreFlag=true", 4)
        }
    }
