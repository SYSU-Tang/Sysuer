...
fun LeaveSlipRoute(
    ...
    BackHandler {
        ...
        else backStack.navigateBack(activity)
    }
    ActivityPager(
        ...
        onNavigationClick = { if (apply) apply = false else backStack.navigateBack(activity) },
        ...
        SectionCard(
            section = SectionData(
                ...
                footerMenus = remember(attachment) {
                    mutableStateListOf(
                        MenuItem(upload, enabled = !hasAttachment) { onUpload(); true },
                        MenuItem(delete, enabled = hasAttachment) { viewModel.deleteAttachment(); true },
                        MenuItem(preview, enabled = hasAttachment) {
                            context.startActivity(
                                Intent(context, BrowserActivity::class.java).setData(
                                    "https://jwxt.sysu.edu.cn/jwxt/reports-register/askLeaveAgg/downloadFile?filePath=${attachment?.getString("filePath")}&fileName=${
