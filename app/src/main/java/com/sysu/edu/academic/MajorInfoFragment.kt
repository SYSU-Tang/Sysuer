...
class MajorInfoFragment : StaggerFragment() {
	var page: Int = 0
	var total: Int = -1
			...
			if (response.getInteger("code") == 200 && response.get("data") != null) {
				...
				if (message.first == 0) {
					...
					data.getJSONArray("rows").forEach { a: Any? ->
						addSection((a as JSONObject).getString("name"), mutableListOf("专业代码", "专业名称", "学制", "修业年限", "学科门类", "学位授予门类"), extractValue(a, arrayOf("code", "name", "educationalSystem", "maxStudyYear", "disciplineCateName", "degreeGrantName")))
					}
				}
			}
		})
