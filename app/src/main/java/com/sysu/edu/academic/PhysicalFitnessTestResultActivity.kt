...
class PhysicalFitnessTestResultActivity : BaseActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		val binding = ActivityPagerBinding.inflate(layoutInflater).apply {
			...
			pager.adapter = adp
			TabLayoutMediator(tabLayout, pager) { tab: TabLayout.Tab?, position: Int -> tab?.text = mutableListOf<String?>("体测成绩", "体育积分", "游泳")[position] }.attach()
		}
		...
		http = HttpManager(object : Handler(this.mainLooper) {
			override fun handleMessage(msg: Message) {
				...
				if (what == -1) config.toast(R.string.no_net_connected)
					...
					if (msg.data.getInt("code") == 302 || Pattern.compile("window\\.location\\.href.+?\"/caslogin\"", Pattern.DOTALL)
						...
						0, 1, 2 -> {
							...
							val matcher = Pattern.compile("<a class=\"weui-cell weui-cell_access\".+?</a>", Pattern.DOTALL)
								.matcher(msg.obj as String)
						...
						4 -> {
							val matcher = Pattern.compile("<a class=\"weui-cell\">.+?</a>", Pattern.DOTALL)
								.matcher(msg.obj as String)
							while (matcher.find()) {
								val matcher1 = Pattern.compile("class=\"left_side\">(.+?)</div>.+?<p>(.+?)</p></div>", Pattern.DOTALL)
									.matcher(matcher.group())
