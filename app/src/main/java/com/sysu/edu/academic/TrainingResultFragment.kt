...
class TrainingResultFragment : Fragment() {
	...
	override fun onCreateView(
		...
		model.message.observe(requireActivity()) { message: CommonUtil.Tuple2<Int, JSONObject> ->
			...
			if (response.getIntValue("code") == 200) {
				if (message.first == 1) {
					...
					data.getJSONArray("rows").forEach { o: Any? ->
						staggerFragment.addSection((o as JSONObject).getString("name"),
						                           R.drawable.book,
						                           mutableListOf("专业", "年级", "学院", "培养类别", "修业年限", "学科门类", "学位", "专业代码", "专业ID"),
						                           extractValue(o, arrayOf("professionName", "grade", "manageUnitName", "trainTypeName", "educationalSystem", "disciplineCateName", "degreeGrantName", "professionCode", "professionId")))
					}
				}
			}
		}
	...
	fun getSelectedCourses(unit: String?, grade: String?, profession: String?, trainType: String?) {
		model.addAndNext("jwxt/training-programe/training-programe/undergradute/profession-info",
		                 "{\"pageNo\":${++page},\"pageSize\":10,\"total\":true,\"param\":{\"manageUnitNum\":\"$unit\",\"grade\":\"$grade\",\"professionCode\":\"$profession\",\"trainTypeCode\":\"$trainType\"}}",
		                 1)
	}
