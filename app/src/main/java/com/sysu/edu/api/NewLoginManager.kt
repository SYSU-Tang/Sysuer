...
class NewLoginManager(private val context: Context) {
	...
	@Throws(Exception::class) fun loginForKTP(username: String, password: String) {
		...
		if (!TextUtils.isEmpty(type) && type!!.contains("application/json")) {
			val result = JSONObject.parse(response.body.string())
			val code = result.getString("code")
			val status = result.getInteger("status")
			if ("1000" == code || status == 1) authorizationJar.setToken("www.ketangpai.com",
	 ...
	 * @param service  登录服务
	 */
	fun login(username: String?, password: String, service: String) {
			...
			try {
				...
				if (service.contains("webvpn")) {
					...
					val redirect = redirect(doLogin(username,
					                                encrypt(publicKey.getString("publicKey"),
					                                        password),
					                                publicKey.getString("publicKeyId")))
					if (redirect == null) return@supplyAsync false
					request("https://webvpn.sysu.edu.cn/users/auth/cas/callback?url", true)
					getWebvpnKey(service)
					when (service) {
						TargetUrl.NEWS_WEBVPN -> setAuthorization(host,
						                                          getNewsAuthorization(service))
						TargetUrl.GYM_WEBVPN -> {
							getGymToken(targetBaseUrl)
							cookieJar.copy(targetBaseUrl, "https://gym.webvpn.sysu.edu.cn")
							val gymAuthorization = getGymAuthorization(targetBaseUrl)
							println(gymAuthorization)
					...
					when (service) {
						TargetUrl.PORTAL -> {
							loginForPortal()
							cookieJar.copy("https://portal.sysu.edu.cn",
							               "https://mportal.sysu.edu.cn")
						}
						TargetUrl.GYM -> {
	...
	internal class CookieStore(val cookieManager: CookieManager) : AndroidCookieJar() {
		...
		fun copy(from: String, to: String) {
			saveFromResponse(from.toHttpUrl(), loadForRequest(to.toHttpUrl()))
		}
