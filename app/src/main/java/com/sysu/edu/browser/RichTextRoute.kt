...
@Composable fun RichTextRoute(
	...
	DataStoreManager.loadContent(LocalContext.current, title) {
		content = it
	}
	ActivityPager(title = title, onNavigationClick = { backStack.navigateBack(activity) }, pageContent = {
		Column(modifier = Modifier
			.fillMaxWidth()
			.padding(dimensionResource(R.dimen.horizontal_padding), dimensionResource(R.dimen.vertical_padding))) {
			if (contentType == DataStoreManager.ContentType.MARKDOWN.name) Markdown(markdownState = rememberMarkdownState(content), colors = markdownColor(), typography = markdownTypography(), modifier = Modifier.fillMaxWidth())
			else RichText {
		...
		SmallExtendedFloatingActionButton(icon = {
			...
			coroutine.launch { clipboard.setClipEntry(ClipData.newPlainText(title, content).toClipEntry()) }
		})
