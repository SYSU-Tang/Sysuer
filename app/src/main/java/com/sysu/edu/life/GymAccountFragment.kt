		...
		model.message.observe(requireActivity(), Observer { message: CommonUtil.Tuple2<Int, JSONObject> ->
			...
			when (message.first) {
				0 -> {
					...
					preferenceAdapter.set(mutableListOf(R.string.type, R.string.name, R.string.student_id, R.string.net_id), extractValue(response, arrayOf("Type", "Name", "HostKey", "UserId")), mutableListOf(R.drawable.help, R.drawable.text, R.drawable.school, R.drawable.id), requireContext())
					...
					cashAdapter.set(mutableListOf(R.string.sport_credit, R.string.wallet), extractValue(response, arrayOf("Credits", "CashWallet")), mutableListOf(R.drawable.dashboard, R.drawable.money), requireContext())
					...
					arrayOf<String?>("validSwimmer", "IsAdmin").forEachIndexed { i, v ->
						idAdapter.add(getString(listOf(R.string.is_swimmer_valid, R.string.admin)[i]), if (response.getBoolean(v)) getString(R.string.yes) else getString(R.string.no), R.drawable.help)
					}
				...
				1 -> response.getJSONArray("data").forEach { i: Any? ->
					...
					val list: ArrayList<String?> = extractValue(item, arrayOf("Status", "ValidUntil", "PhysicalExamDate"))
					list[0] = if ("approved" == list[0]) getString(R.string.approved) else getString(R.string.disapproved)
					certAdapter.set(mutableListOf(R.string.status, R.string.valid_date, R.string.physical_exam_date), list, mutableListOf(if ("approved" == list[0]) R.drawable.uncheck else R.drawable.check, R.drawable.calendar, R.drawable.calendar), requireContext())
