...
class NetPayViewModel(application: Application) : AndroidViewModel(application) {
	...
	private val status2Keys = mutableListOf<String?>("序号", "服务", "校区（园）", "地址", "MAC地址", "部门", "使用者", "状态", "过期日期", "续费时长", "金额")
	
	init {
		model.message.observeForever { (code, data) ->
			when (code) {
				0, 1, 6 -> {
					parse(data.getString("data")).selectFirst("tbody")?.select("tr")?.forEach { tr ->
						...
						when (code) {
							0 -> {
								...
								if (payLink != null) {
									val orderId = row[0] ?: ""
									section.footerMenus.add(MenuItem(getString(R.string.pay_fee), null) {
										continuePay(orderId)
										true
									})
									section.footerMenus.add(MenuItem(getString(R.string.cancel_pay), null) {
										cancelPay(orderId)
										true
									})
								}
							...
							1 -> {
								...
								if (actionLink != null) {
									...
									if (match != null) {
										...
										section.footerMenus.add(MenuItem(actionLink.text(), null) {
											showStopOrResumeConfirmation(serviceId, isStop, leftDay)
											true
										})
										section.footerMenus.add(MenuItem(getString(R.string.pay_fee), null, isStop) {
											openPayDialog(serviceId, row[1] ?: "", row[7] ?: "")
											true
										})
							...
							6 -> {
								...
								section.footerMenus.add(MenuItem(getString(R.string.pay_fee), null, serviceId.isNotEmpty()) {
					...
					if (!location.isNullOrEmpty()) {
						model.http.handler.post {
							...
							val intent = Intent.createChooser(
								Intent(Intent.ACTION_SEND).setType("text/plain").putExtra(Intent.EXTRA_TEXT, location).putExtra(Intent.EXTRA_SUBJECT, getString(R.string.recharge)).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK),
	...
	private fun getSection(title: String?, keys: List<String?>, values: List<String?>): SectionData {
