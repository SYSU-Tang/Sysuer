...
public class NewsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (savedInstanceState == null) {
            ...
            binding.recyclerView.setLayoutManager(new GridLayoutManager(requireContext(), config.getColumn()));
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            ...
            if (!img.isEmpty())
                Glide.with(context).load(new GlideUrl(img, new LazyHeaders.Builder()
                                ...
                                .build()))
                        .timeout(15000)
                        .override(config.dpToPx(120), config.dpToPx(120)).optionalFitCenter().transform(new RoundedCorners(16))
                        .into(binding.image);
