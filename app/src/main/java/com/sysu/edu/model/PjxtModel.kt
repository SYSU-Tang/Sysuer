...
class PjxtModel(context: Context) {
	...
	fun login(request: CommonUtil.Tuple2<Request, Int>) {
		afterLoginRequest.isEmpty().let {
			...
			if (it || request in afterLoginRequest) contextUtil.login(
				if (authorizationManager.isAccessible()) TargetUrl.PJXT else TargetUrl.PJXT_WEBVPN
			) {
