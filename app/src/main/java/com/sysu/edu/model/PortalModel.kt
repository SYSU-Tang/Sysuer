...
class PortalModel(context: Context) {
    ...
    private val http = HttpManager(Handler(Looper.getMainLooper()))
    ...
    init {
        http.setCookieManager(CookieManager(context))
    }
    ...
    fun login(request: CommonUtil.Tuple2<Request?, Int?>?) {
        afterLoginRequest.isEmpty().also {
            afterLoginRequest.add(request)
            if (it) contextUtil.login(TargetUrl.PORTAL) {
                afterLoginRequest.forEach { request -> this.retry(request!!) }
            }
        }
    ...
    fun updateRequest(request: Request): Request {
        return request.newBuilder()
            .header("Cookie", http.getCookieManager().toSimpleString("portal.sysu.edu.cn")).build()
    }
