...
public class XgxtModel {
    ...
    public void login(CommonUtil.Tuple2<Request, Integer> request) {
        ...
        if (empty)
            contextUtil.login(authorizationManager.isAccessible() ? TargetUrl.XGXT : TargetUrl.XGXT_WEBVPN, () -> afterLoginRequest.forEach(this::retry));
                ...
                switch (code) {
                    ...
                    case 200 -> {
                        ...
                        if (type != null && type.contains("application/json")) {
                            ...
                            JSONObject meta;
                            if (contentJSON.containsKey("meta") && !(meta = contentJSON.getJSONObject("meta")).getBoolean("success")) {
                                if (meta.containsKey("statusCode") && meta.getInteger("statusCode").equals(302))
                                    login(request);
                                else
                                    http.getHandler().post(() -> contextUtil.toast(toStringOrDefault(meta.getString("message"))));
                            } else {
                                if (contentJSON.containsKey("code") && contentJSON.getInteger("code") != 200)
                                    http.getHandler().post(() -> contextUtil.toast(toStringOrDefault(contentJSON.getString("msg"))));
